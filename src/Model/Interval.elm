...
compare (Interval intA) (Interval intB) =
   if (Date.compare intA.start intB.start) == LT then LT
    else if (Date.compare intA.start intB.start) == GT then GT
        else case (intA.end, intB.end) of  -- equal start dates
           ( Just startA, Just startB ) -> 
                                     Date.compare startA startB
...
view interval =
    let
        ...
        duration = case length interval of
                    Just (years, months) ->
                         ("Duration: " ++ String.fromInt years ++ " years and " ++ String.fromInt months ++ " months") 
    ...
    in
        div [ class "interval" ]
         [
            div [ class "interval-start" ] [ text "Start date: ", Date.view (int.start)]
            , div [ class "interval-end" ] [ text "End date: ", Maybe.map Date.view (int.end) |> Maybe.withDefault (text "Present") ]
            , div [ class "interval-length" ] [ text duration ]
        ]
