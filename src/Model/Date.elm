...
monthsBetween dA dB =
    let
        ...
        yearDifference = abs (d2.year - d1.year)
        monthDifference =
            case (d1.month, d2.month) of
                ...
                ( Just m1, Just m2 ) ->
                    let
                        monthsD1 = 12 * d1.year + monthToInt m1
                        monthsD2 = 12 * d2.year + monthToInt m2
                    in
                    abs (monthsD2 - monthsD1)
                _ -> -1 
    in
    if monthDifference >= 0 then
        Just monthDifference
    else
        Nothing
...
compare (Date d1) (Date d2) =
    if d1.year < d2.year then LT
            ...
            else case ( d1.month , d2.month ) of  -- equal months
                ( Just m1 , Just m2)  -> 
                                            compareMonth m1 m2
...
view (Date d) =
    div []
        [ 
         text ( String.fromInt d.year ++ " " ++ Maybe.withDefault "" (Maybe.map monthToString d.month ) ) 
        ]
